...
def plot_field(df,var):
  ...
  for e,expid in enumerate(expids):
    ...
    # plot map 
    m = Basemap(lon_0=180,ax=ax,llcrnrlat=-20,urcrnrlat=60,llcrnrlon=80,urcrnrlon=240)
    ...
    # plot countourf
    im = ax.contourf(x, y, data, levels, cmap='RdYlBu_r',  alpha=0.75)
    c = ax.contour(x, y, data, clevels, colors='k', linewidths=0.5)
    plt.clabel(c,inline=True, fontsize=10, fmt='%5.1f')
